...
KernelAgent 卡片（`config-tool`）提供“生成后自动优化”开关（`autoOptimize`，默认 `false`）。开启后，生成结果通过正确性验证就会进入性能优化。“生成纠错轮数”（`generationMaxRounds`，默认 `8`）和“性能优化轮数”（`maxRounds`，默认 `8`）独立设置。优化失败时保留已验证的生成源码并报告优化错误；优化成功时显示实测耗时和加速比。发起新的生成请求前需保存卡片设置。
