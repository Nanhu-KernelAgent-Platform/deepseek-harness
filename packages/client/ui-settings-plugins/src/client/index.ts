...
import { KernelAgentConfigCard } from './KernelAgentConfigCard.tsx'
...
import { CONFIG_TOOL_NS, ConfigToolCardController } from './kernelagent-config-card-controller.ts'
...
export function apply(ctx: ClientContext): void {
  ...
  const configTool = new ConfigToolCardController(ctx.settingsScope.bind({ namespace: CONFIG_TOOL_NS }))
  ...
  ctx.slots.inject('settings.plugin.item', function* () {
    ...
    yield ctx.slots.register({
      ...
      key: CONFIG_TOOL_NS,
      locale: NS,
      inject: () => configTool.inject(),
    }, KernelAgentConfigCard)
  })
}
